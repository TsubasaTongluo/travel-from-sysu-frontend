...
const getNoteList = async (type: number) => {
  if (type === 1) {
    ...
    try {
      ...
      notelist.value = await Promise.all(res.data.data.notes.map(async (note: any) => {
        let new_note: Note = { ...note };

        // 将 note.urls 解析为数组
        if (note.note_URLs) {
          try {
            new_note.note_URLs = JSON.parse(note.note_URLs); // 如果是 JSON 字符串，解析为数组
          } catch (error) {
            console.log("解析 urls 失败:", error);
            new_note.note_URLs = []; // 如果解析失败，设置为一个空数组
          }
        } else {
          new_note.note_URLs = []; // 如果没有 urls 字段，默认是空数组
        }

        new_note.noteType = "image";
